...
/**
 * Poll application based on ActiveJ + Rocker template + Redis
    ...
    AsyncServlet servlet(PollDao pollDao) {
        ...
        return RoutingServlet.create()
                ...
                .map(HttpMethod.GET, "/poll/:id", request -> {
                    final BindableRockerModel singlePollView = Rocker.template("views/singlePollView.rocker.html");
                ...
                .map(HttpMethod.POST, "/vote", request -> request.loadBody()
                        .then(() -> {
                            ...
                            question.then(poll -> Promise.of(poll.vote(option)))
                ...
                .map(HttpMethod.POST, "/add", request -> request.loadBody()
                        ...
                        .then($ -> {
                            ...
                            return result.map(id -> HttpResponse.redirect302("poll/" + id))
                                    .whenException(Exception::printStackTrace);
                        }))
                .map(HttpMethod.POST, "/delete", request -> request.loadBody()
                        .then(() -> {
                            ...
                            Promise<Void> result = pollDao.remove(Integer.parseInt(id));
                            result.whenResult($ -> System.out.println("DEL Complete"));
